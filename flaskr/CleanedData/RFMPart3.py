...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 13:57:18 2019
...
df = pd.read_csv('FinalStockTransaction.csv', parse_dates=['trans_date'])
df.head(3)
...
rfmTable = df.groupby('customer_id').agg({
                        ...
                        'MonetryValue':  lambda x: x.abs().sum()           # Monetary Value
                        })
rfmTable.rename(columns = {'hist': 'recency',
                           'customer_id': 'frequency',
                           'MonetryValue': 'monetary_value'}, inplace = True)
